...
async function requireScheduling(request: Request, _response: Response, next: NextFunction) {
  try {
    ...
    const [rows] = await pool.query<RowDataPacket[]>(
      "SELECT enabled FROM tenant_systems WHERE client_id = :clientId AND system_code = 'scheduling' LIMIT 1",
      { clientId }
    );
    if (!rows[0]?.enabled) throw new AppError(403, "Scheduling system is disabled for this tenant");
    next();
  } catch (error) {
    next(error);
  }
